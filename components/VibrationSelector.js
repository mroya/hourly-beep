...
import { VIBRATION_PATTERNS, playVibrationPreview, triggerHapticFeedback } from '../services/vibration';
import { trackEvent, EVENTS } from '../services/analytics';
import { COLORS, SPACING, RADIUS } from '../styles/theme';

export default function VibrationSelector() {
  const { selectedVibration, isEnabled, isPremium } = useAppState();
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                dispatch({ type: ACTIONS.SET_VIBRATION, payload: pattern.id });
                trackEvent(EVENTS.VIBRATION_CHANGED, { vibration: pattern.id });
              }}
            ...
            >
              ...
              <Feather name={pattern.icon} size={18} color={isActive ? COLORS.neonCyan : COLORS.textMuted} style={styles.vibrationIcon} />
              ...
              {pattern.id !== 'off' && !isLocked && (
                ...
                >
                  <Feather name="activity" size={14} color={isActive ? COLORS.neonCyan : COLORS.textMuted} />
                </TouchableOpacity>
              )}
              {isActive && !isLocked && (
